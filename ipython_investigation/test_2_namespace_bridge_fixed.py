#!/usr/bin/env python3
"""Test 2: Namespace bridging between IPython and PyREPL3's NamespaceManager - FIXED."""
...
def test_namespace_bridging():
    ...
    try:
        ...
        # Initialize shell properly first
        shell.init_create_namespaces()
        
        # Point IPython to our managed namespace
        # Note: We can only set user_ns, not user_global_ns
...
def test_transaction_support():
    ...
    try:
        ...
        # Update IPython's namespace with our manager's namespace
        shell.user_ns.update(ns_manager.namespace)
        
        # Set initial state
        shell.run_cell("initial_state = 'original'")
        ns_manager.update(shell.user_ns)  # Sync back
        ...
        # Re-sync IPython with restored namespace
        shell.user_ns.clear()
        shell.user_ns.update(ns_manager.namespace)
...
def test_async_namespace_sync():
    ...
    try:
        ...
        async def test_async_sync():
            # Execute async code - FIX: include asyncio import
            code = """
import asyncio
...
"""
            # Pass transformed_cell to avoid deprecation warning
            result = await shell.run_cell_async(code, transformed_cell=code)
            
            # Sync namespace
            ns_manager.update(shell.user_ns)
...
def test_namespace_persistence():
    ...
    try:
        ...
        for i in range(3):
            # Clear any existing instance for each cycle
            if InteractiveShell.initialized():
                InteractiveShell.clear_instance()
...
def test_namespace_isolation():
    ...
    try:
        ...
        # Create first shell
        shell1 = TerminalInteractiveShell.instance()
        shell1.init_create_namespaces()
        shell1.user_ns.update(ns_manager1.namespace)
        
        # For second shell, we need to work around singleton
        # Save reference and clear
        InteractiveShell.clear_instance()
        
        # Create second shell
        shell2 = TerminalInteractiveShell.instance()
        shell2.init_create_namespaces()
        shell2.user_ns.update(ns_manager2.namespace)
        
        # Execute different code in each
        shell1.run_cell("session_id = 'session1'")
        ns_manager1.update(shell1.user_ns)
        
        shell2.run_cell("session_id = 'session2'")
        ns_manager2.update(shell2.user_ns)
...
def main():
    ...
    print("IPython Integration Investigation - Namespace Bridging (FIXED)")
    print("=" * 60)
