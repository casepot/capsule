#!/usr/bin/env python3
"""Test 5: Performance, cancellation, and IPython event system integration - FIXED."""
...
"""
        ...
        # Calculate overhead
        if direct_metrics.execution_time > 0:
        ...
        # Reasonable overhead threshold (< 100% - IPython shouldn't take more than 2x)
        acceptable_overhead = time_overhead < 100
...
def test_async_performance():
    """Test async execution performance - FIXED."""
    print("\n" + "=" * 60)
...
"""
            ...
            # Measure async performance properly
            start_time = time.time()
            await shell.run_cell_async(async_code, transformed_cell=async_code)
            end_time = time.time()
            ...
            # Should take ~0.1s (10 * 0.01s sleep) plus some overhead
            # Allow up to 1 second total (generous for CI/slow machines)
            reasonable_time = execution_time < 1.0
...
def test_memory_management():
    """Test memory management and namespace cleanup - FIXED."""
    print("\n" + "=" * 60)
    ...
    try:
        ...
        # Memory should be mostly recovered (within 50% of the increase)
        # This is more realistic than expecting full recovery
        memory_increase = after_creation - baseline_memory
        memory_remaining = after_cleanup - baseline_memory
        memory_recovered = memory_remaining < (memory_increase * 0.5)
        print(f"✓ Memory mostly recovered: {memory_recovered}")
        ...
        # Should have fewer variables after reset
        namespace_cleared = reset_size < namespace_size / 2
        print(f"✓ Namespace properly reduced: {namespace_cleared}")
...
def test_concurrent_execution():
    ...
    try:
        ...
        for i in range(3):
            ...
            # Important: Clear instance for next iteration
            InteractiveShell.clear_instance()
        ...
        # Run sequentially since IPython isn't thread-safe
        codes = [
            "result = 'shell0'",
            "result = 'shell1'",  
            "result = 'shell2'",
        ...
        for i, (shell, code) in enumerate(zip(shells, codes)):
            execute_in_shell(shell, code, i)
        
        print(f"✓ Execution results: {results}")
        
        # Check that we got different results (simulated isolation)
        all_different = len(set(r[1] for r in results)) == len(results)
        print(f"✓ Results are different: {all_different}")
...
def main():
    ...
    print("IPython Integration Investigation - Performance & Events (FIXED)")
    print("=" * 60)
