#!/usr/bin/env python3
"""Test 4: Capability injection and security with IPython."""
...
def test_capability_injection():
    ...
    try:
        ...
        async def test_async_cap():
            result = await shell.run_cell_async("result = await fetch('http://example.com')")
...
def test_security_policy_enforcement():
    ...
    try:
        ...
        for level in [SecurityLevel.SANDBOX, SecurityLevel.RESTRICTED, SecurityLevel.STANDARD]:
            ...
            # Create new namespace for this security level
            shell.user_ns.clear()
            shell.reset()
...
def test_dynamic_capability_management():
    ...
    try:
        ...
        # Use it
        async def test_use():
            await shell.run_cell_async("result = await fetch('http://test.com')")
...
def test_ipython_preprocessor_security():
    """Test using IPython preprocessors for security."""
    print("\n" + "=" * 60)
    ...
    try:
        ...
        for code, keyword in blocked_tests:
            try:
                ...
                print(f"✓ Blocked {keyword}: {str(e)}")
                blocked_works = True
            except:
                # May fail at execution if preprocessing doesn't catch it
                blocked_works = False
        ...
        # Note: Preprocessor approach has limitations
        print("\nNote: IPython preprocessors have limitations for security")
        
        return safe_works  # Partial success expected
...
def main():
    ...
    print("IPython Integration Investigation - Capabilities & Security")
    print("=" * 60)
