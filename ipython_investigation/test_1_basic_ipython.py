#!/usr/bin/env python3
"""Test 1: Basic IPython InteractiveShell capabilities and requirements."""
...
def test_basic_shell_creation():
    ...
    try:
        ...
        shell.user_ns = custom_ns
        shell.push({'added': 'value'})
        print(f"  - Can set custom namespace: {custom_ns.get('added') == 'value'}")
...
result
"""
            result = await shell.run_cell_async(code_with_await)
            return result
...
def test_io_override():
    ...
    try:
        ...
        result = shell.run_cell("response = input('prompt>')")
...
def main():
    """Run all tests."""
    print("IPython Integration Investigation - Basic Tests")
    print("=" * 60)
