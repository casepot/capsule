...
class FrameReader:
    ...
    async def read_frame(self, timeout: Optional[float] = None) -> bytes:
        ...
        async with self._condition:
            ...
            logger.debug(f"Phase 2: Waiting for {total_needed} bytes total (have {len(self._buffer)})")
            try:
                await asyncio.wait_for(
                ...
                logger.debug(f"Timeout waiting for frame data (needed {total_needed}, have {len(self._buffer)})")
                raise
            ...
            logger.debug(f"Frame extracted: {len(frame)} bytes, buffer remaining: {len(self._buffer)} bytes")
...
class MessageTransport:
    ...
    async def send_message(self, message: Message) -> None:
        ...
        # Send frame
        logger.debug(f"MessageTransport: sending frame of {len(data)} bytes for message type={message.type}")
    ...
    async def receive_message(self, timeout: Optional[float] = None) -> Message:
        ...
        logger.debug("Received message", type=message.type, id=message.id, parsed_type=type(message).__name__)
