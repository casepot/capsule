...
- Key changes:
  ...
  - Cancellation and timeout notes: `CancelledError` and `TimeoutError` are annotated with `execution_id`, `mode`, optional `cancel_reason`, an optional timestamp, and the first ~160 chars of code.
